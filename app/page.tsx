...
export default function WeddingCardPage() {
  ...
  const handleRsvpSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        formData.append("attendance", rsvpForm.attendance === "yes" ? "Sẽ đến" : "Không thể đến");
        ...
        alert(`Cảm ơn ${rsvpForm.name} đã gửi phản hồi!`);
        setIsRsvpOpen(false);
      ...
      <div className={`relative selection:bg-[#E5D9CC] selection:text-[#4A3C31] font-sans text-[#5C4F44] bg-[#8C8076] w-full flex flex-col items-center mx-auto overflow-hidden h-[100dvh] transition-opacity duration-[1200ms] ease-in-out ${isPageLoaded ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="w-full h-[100dvh] flex justify-center items-center p-0 md:p-6 relative">
            ...
            <div className="relative w-full max-w-[460px] h-full max-h-[850px] shadow-2xl md:rounded-lg border-x border-[#EAE3DB] overflow-hidden bg-[#FDFBF7]" style={{ perspective: '2000px' }}>
                ...
                {/* BÌA THIỆP */}
                <div 
                    className={`absolute inset-0 w-full h-full bg-[#FDFBF7] z-50 flex flex-col items-center justify-center text-center px-6 relative overflow-hidden transition-all duration-[1500ms] ease-[cubic-bezier(0.645,0.045,0.355,1)] ${cardState === 'done' ? 'hidden' : ''}`}
                    style={{ 
                ...
                >
                    <div className="absolute inset-0 w-full h-full flex items-center justify-center pointer-events-none z-10 p-4">
                        <img src="/bia.png" alt="Bìa" className="w-full h-full max-h-[92dvh] object-contain select-none" onError={(e) => { if (!e.currentTarget.src.includes('.jpg')) e.currentTarget.src = "/bia.jpg"; }} />
                    </div>
                   ...
                   <FadeIn threshold={0.05} className="relative w-full flex justify-center mt-4 mb-20 px-2">
                       ...
                       <div className="relative w-[95%] max-w-[400px] art-paper-bg rounded-sm shadow-[0_15px_40px_rgba(0,0,0,0.08)] border border-[#EAE3DB]">
                           <div className="px-5 md:px-6 pt-16 pb-12 flex flex-col items-center text-center relative z-20 w-full">
                               ...
                               <button className="text-[#8C7A6B] text-[11px] md:text-[12px] font-medium underline underline-offset-4 mb-6 hover:text-[#5C4F44] transition-colors relative z-[100]">
                                  Thêm vào lịch
                               </button>
                               ...
                               <button 
                                  ...
                                  className="bg-[#77665B] text-white px-8 py-3.5 rounded-full text-[10px] md:text-[11px] tracking-[0.2em] uppercase shadow-[0_4px_15px_rgba(119,102,91,0.4)] hover:bg-[#5C4F44] active:scale-95 transition-all relative z-[999] cursor-pointer"
                               >
                                  Xác nhận tham dự
                               </button>
